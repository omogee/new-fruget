  ...
  const [selectedimage, setselectedimage] = context["selectedimage"]
   ...
   useEffect(()=>{
    if(Cookies.get("tktplc")){
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/dispatch_status_view?tkt=${Cookies.get("tktplc")}`)
        .then(res => {
    ...
    if(query.get("confirmorder") && didMount.current){
        ...
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/confirm_rateorder?skb=${query.get("skb")}&pid=${query.get("pid")}&clear=${query.get("single")}&tkt=${Cookies.get("tktplc")}&confirmId=${query.get("confirmorder")}`)
        .then(res =>{
            if(res.data.status === "success"){
...
useEffect(()=>{
    ...
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_dispatchreciepts?tkt=${Cookies.get("tktplc")}&order=${query.get("order") || order}`)
    .then(res =>{
        if(res.data.status === "success"){
...
const clearsingleorder =(data)=>{
    setloading(true)
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/clear_singleorder?cartId=${data.cartId}&productId=${data.productId}&tkt=${Cookies.get("tktplc")}`)
    .then(res =>{     
        if(res.data.status === "success"){
            ...
            axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_submitted_cart?tkt=${Cookies.get("tktplc")}`)
            .then(res =>{
                if(res.data.status === "success"){
...
const clearallorder =()=>{
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/clear_multipleorder?tkt=${Cookies.get("tktplc")}`)
    .then(res =>{
        if(res.data.status === "success"){
...
const opendetails=(properties)=>{
  ...
  const clearorder=()=>{
    if(query.get("confirmorder") && didMount.current){
        ...
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/confirm_clearorder?store=${store}&storedetails=${JSON.stringify(storedetails)}&dispatchratings=${JSON.stringify(dispatchratinginputs)}&storeratings=${JSON.stringify(ratinginputs)}&skb=${query.get("skb")}&pid=${query.get("pid")}&clear=${query.get("single")}&tkt=${Cookies.get("tktplc")}&confirmId=${query.get("confirmorder")}`)
        .then(res =>{
            if(res.data.status === "success"){
                setTimeout(()=> setloading(false), 800)
                setdisplayratingmodal(false)
  ...
  const openinvoice=(data)=>{
     ...
     axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_ungroupeddispatchreciepts?tkt=${Cookies.get("tktplc")}&invoiceNo=${data.invoiceNo}&customerId=${data.customerId}&storeId=${data.storeId}&order=${query.get("order") || order}`)
     .then(res =>{
         if(res.data.status === "success"){
