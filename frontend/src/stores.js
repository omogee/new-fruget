...
const submitprofile =()=>{
 ...
 formdata.append("files", profilepicture)
  ...
  axios.post(`https://new-frugetbackend-production.up.railway.app/client/edit_profile`, formdata)
  .then( res => {
...
const sendconfirmation=()=>{
   setloading(true)
   axios.get(`https://new-frugetbackend-production.up.railway.app/client/confirm_registered_email?email=${userdetails.email}&customerId=${userdetails.customerId}`)
   .then(res =>{
    if(res.data.status === "success"){
...
const agreedchange=(data)=>{
 ...
 <div className='col-6 mb-2'>
    ...
    Registered Stores ({registeredstores.length})
    </small>
 </div>
 <div className='col-6 mb-2'>
    <small onClick={()=> setdisplayterms(prev => !prev)} style={{cursor:"pointer",fontWeight:"bold",color:"grey",padding:"2px 10px", borderRadius:"10px",border:"1px solid grey",marginRight:'30px', float:'right'}}>
    <span className='fa fa-plus-circle'> Add store</span>
    </small>
 </div>
