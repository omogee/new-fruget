    ...
    const [groups, setgroups] = useState([])
      ...
      const querystring = new URLSearchParams(window.location.search)
        ...
        if(Cookies.get("tktplc")){
          ...
          axios.get(`https://new-frugetbackend-production.up.railway.app/fetch-connections?tkt=${Cookies.get("tktplc")}`)
          .then(res => {
...
useEffect(()=>{
  ...
  // axios.get(`https://new-frugetbackend-production.up.railway.app/fetch-connections?id=${ownerid}`)
  // .then(res => {
  //   setconn(res.data.connections)
  // })
  // .catch(err => console.warn(err))
