...
<select type="text" style={{border:`1px solid ${inputs.generalcategorycolor}`}} id="generalcategory" name="generalcategory" onChange={change} className="form-control" value={inputs.generalcategory}  placeholder="Enter Category e.g home Appliance,clothing, accessories etc">
   <option value="">select general category</option>
    {Object.keys(category).map(cat =>
            <option value={`${cat}`}>{cat}</option>
                        ...
                        {inputs.generalcategory && inputs.generalcategory.length > 0 ?
                        <div className="col-6">
                        <div style={{padding:"10px"}}>
                            <label htmlFor="category">Category</label>
   <select style={{backgroundColor:"white",border:`1px solid ${inputs.categorycolor}`}} type="text" onChange={change} id="category" name="category" className="form-control" value={inputs.category} >
   <option value="">select category</option>     
                         {category[`${inputs.generalcategory}`] && category[`${inputs.generalcategory}`].map(subcat=>
                 ...
                 <input type="file" multiple className="form-control" onChange={changefile}  name="files"/>
                      ...
                      <span className="fa fa-times text-danger" onClick={()=>this.popImage(file.imgfile)} style={{fontSize:"15px",fontWeight:"bolder"}}></span>
                       <img className='animated bounce' style={{width:"100%",height:"130px",borderRadius:"5px",border:'1px solid lightgrey',padding:'2px'}} src={file.img} alt=""/>
                        ...
                        {inputs.category && inputs.category.length > 0 ?
                        <div className="col-6">
                        <div style={{padding:"10px"}}> 
                            <label htmlFor="subcat1">Select sub-category</label>
                            <select className="form-control" onChange={change} value={`${inputs.subcat1}`} name="subcat1" id="subcat1">
                           {category[`${inputs.generalcategory}`] && category[`${inputs.generalcategory}`].map(cat=>
                             cat.name === inputs.category ?
                        ...
                        <div className="col-6">
                            ...
                            <select className="form-control" id="subcat2" name="subcat2" onChange={change} value={`${inputs.subcat2}`}>
                            {category[`${inputs.generalcategory}`] && category[`${inputs.generalcategory}`].map(cat=>
                             cat.name === inputs.category ?
                        ...
                        <div className="col-6">
                            ...
                            <select className="form-control" name="subcat3" id="subcat3" onChange={change} value={`${inputs.subcat3}`}>
                            {category[`${inputs.generalcategory}`] && category[`${inputs.generalcategory}`].map(cat=>
                             cat.name === inputs.category && cat.subcat3?
         ...
         <input type="text" id="brand"  name="brand" onChange={change} className="form-control" value={inputs.brand} placeholder="Note : No spacing inbetween E.g lg, hisense, tummyhilfiger"/>
                        </div>
                        </div>
                        <div className="col-6">
                             <div style={{padding:"10px"}}>
                            <label htmlFor="entrytext">Entry Text</label>
<input type="text" id="entrytext"  name="entrytext" onChange={change} className="form-control" value={inputs.entrytext} placeholder="convince your seller in less than 100 words.."/>
<small className="text-muted" style={{fontSize:"10px"}}>get off the wether with quality and affordable jackets from tummyhilfiger</small>
    ...
    <input type="text"  id="details" name="details" style={{border:`1px solid ${inputs.detailscolor}`}} onChange={change} className="form-control" value={inputs.details} placeholder="e.g bianco standing fan, lg copper double door refrigerator...etc(50)"/>
                        </div>                     
                        {inputs.details && searchinput.length > 0 && inputs.details.length && products.length > 0 ?
                    <div style={{position:"absolute",zIndex:"685845",padding:"5px",boxShadow:"2px 2px 3px 3px lightgrey",backgroundColor:'white',overflow:"hidden", height:"200px",width:"100%"}}>
                          <small className="text-muted">This product already exist on fruget community...Click to auto file columns</small>   
                     <div style={{width:"100%",height:"100%",marginBottom:"20px",padding:"10px",overflow:"scroll"}}>
                     {products.map(prod =>
                        <p  className='mb-2' onClick={()=>fillform(prod)} style={{cursor:"pointer",margin:"3px",lineHeight:"2vh",fontSize:'13px'}}>
                            <span>{prod.details.length > 60 ? prod.details.slice(0,60) + "..." : prod.details}</span><br/>
                            <small><b>{prod.generalcategory}</b>, <b>{prod.category}</b>, <b>{prod.subcat1}</b>, <b>{prod.model || prod.subcat2}</b>, <b style={{color:"grey"}}>{prod.color}</b> ...</small>
                        </p>
                        )}
                     </div>
                    </div>    
                    : null}
                        ...
                        <div style={{padding:"10px"}}>
                         ...
                         <input type="text" id="colors" name="color" onChange={change} style={{fontSize:"14px"}} className="form-control" value={inputs.color} placeholder="Enter all the colours available in your store seperated by commas ( , )"/>
 ...
 <input type="text" id="size" name="size" style={{fontSize:"14px"}} onChange={change} className="form-control" value={inputs.size} placeholder="Enter all the sizes available in your store seperated by commas ( , )"/>
 ...
 <input type="text" id="megapixel" name="megapixel" style={{fontSize:"14px"}} onChange={change} className="form-control" value={inputs.megapixel} placeholder="Enter size of the this product"/>
                        ...
                        <div style={{padding:"10px"}}>
                         ...
                         <input type="text" id="price" name="price" className="form-control" onChange={change} value={inputs.price}/>
    <small className="text-danger">Kindly enter the best release price as prices on fruget are not negotiable</small>
                        ...
                        <div style={{padding:"10px"}}>
                         ...
                         <input type="text" id="initialprice" name="initialprice" className="form-control" onChange={change} value={inputs.initialprice}/>
   ...
   <input type="text" id="model" name="model" className="form-control" onChange={change} value={inputs.model} placeholder="Enter the model number for easier identification"/>
                        </div>
                        </div>
                        <div className="col-6 col-md-3">
                        <div style={{padding:"10px"}}>
                            <label htmlFor="power">power</label>
 <input type="text" id="power" name="power" className="form-control" onChange={change} value={inputs.power} placeholder="Enter the wattage e.g 20w, 30w, 50w etc"/>
                        </div>
                        </div>
 ...
 <input type="text" id="weight" name="weight" className="form-control" onChange={change} value={inputs.weight} placeholder="Enter weight in Kg"/>
                        </div>
                        </div>
                        <div className="col-6">
                        <div style={{padding:"10px"}}>
                            <label htmlFor="aboutbrand">About Brand </label>
         <textarea  id="aboutbrand" className="form-control" onChange={change} name="aboutbrand" value={inputs.aboutbrand} placeholder="Enter the wattage e.g 20w, 30w, 50w etc"/>
