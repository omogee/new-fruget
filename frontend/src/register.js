...
function Register() {
   ...
   useEffect(()=>{
    if(query.get("email") && query.get("step")){
        setloading(true)
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/confirm_email/stepTwo?email=${query.get("email")}&step=${query.get("step")}`)
        .then(res => {
...
*/
 const save =()=>{
    if(stepOne){
        ...
        setPassworderror(false)
       setloading(true)
       axios.get(`https://new-frugetbackend-production.up.railway.app/client/confirm_email?email=${inputs.email}&password=${inputs.password}&newsupdatesubscription=${newsupdatesubscription}`)
       .then(res =>{
       ...
       setloading(true)
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/second_form/stepTwo?step=${query.get("step")}&name=${inputs.name}&contact=${inputs.contact}&gender=${inputs.gender}`)
        .then(res =>{
            if(res.data.status === "success"){
                Cookies.remove("cvyx")
                Cookies.remove("cvyxx")
  ...
  axios.post(`https://new-frugetbackend-production.up.railway.app/client/third_form/stepThree`, formdata)
  .then( res => {
   if(res.data.status === "success"){
    Cookies.remove("cvyx")
    Cookies.remove("cvyxx")
...
(error) => {
 ...
 const resend=()=>{
    setloading(true)
    axios.get(`https://new-frugetbackend-production.up.railway.app/resend_mail?email=${inputs.email}&password=${inputs.password}`)
    .then(res => {
        if(res.data.status === "success"){
            setshowtimer(true)
            setdisplayemailsent(true)
