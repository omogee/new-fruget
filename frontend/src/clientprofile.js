...
useEffect(()=>{
 ...
 // var decryptedData = bytes.toString(CryptoJS.enc.Utf8);
  ...
  if(Cookies.get("tktplc")){
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_client?clientId=${params.customerId}`)
    .then(res =>{
...
const followstore=(store)=>{
  ...
  .then(res => {
    if(res.data.status === "success"){
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_client?clientId=${JSON.parse(decryptedData)}`)
    .then(res =>{
       if(res.data.status === "success"){
        setclient(res.data.clientdetails[0])
        setclientstore(res.data.clientstore)
 ...
 const followdispatch=(dispatchId)=>{
  setloading(true)
  axios.get(`https://new-frugetbackend-production.up.railway.app/client/follow_dispatch?dispatchId=${escape(dispatchId)}&tkt=${Cookies.get("tktplc")}`)
  .then(res => {
    if(res.data.status === "success"){
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_client?clientId=${JSON.parse(decryptedData)}`)
    .then(res =>{
 ...
 const unfollowdispatch=(dispatchId)=>{
  setloading(true)
  axios.get(`https://new-frugetbackend-production.up.railway.app/client/unfollow_dispatch?dispatchId=${escape(dispatchId)}&tkt=${Cookies.get("tktplc")}`)
  .then(res => {
    if(res.data.status === "success"){
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_client?clientId=${JSON.parse(decryptedData)}`)
    .then(res =>{
