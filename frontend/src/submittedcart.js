  ...
  console.log("ratings", ratinginputs)
   ...
   useEffect(()=>{
    if(query.get("confirmorder") && didMount.current){
        ...
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/confirm_rateorder?invoiceNo=${query.get("skb")}&pid=${query.get("pid")}&clear=${query.get("single")}&tkt=${Cookies.get("tktplc")}&confirmId=${query.get("confirmorder")}`)
        .then(res =>{
...
const clearsingleorder =(data)=>{
    setloading(true)
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/clear_singleorder?invoiceNo=${data.invoiceNo}&productId=${data.productId}&tkt=${Cookies.get("tktplc")}`)
    .then(res =>{     
        if(res.data.status === "success"){
            axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_submitted_cart?tkt=${Cookies.get("tktplc")}`)
            .then(response =>{
