  ...
  useEffect(()=>{
      ...
      document.querySelector(".overalldiv").addEventListener("click", ()=>{
        ...
        if(!query.get("gby_z") || setting === "list"){
          setloading(true)
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_shoppingcart?tkt=${Cookies.get("tktplc")}`)
        .then(res =>{
    ...
    useEffect(()=>{
      ...
      if(query.get("gby_z") || setting==="grid"){
        setsetting("grid")
          setloading(true)
        axios.get(`https://new-frugetbackend-production.up.railway.app/client/fetch_groupedshoppingcart?tkt=${Cookies.get("tktplc")}`)
        .then(res =>{
         if(res.data.status === "success"){
            setshoppingcart(res.data.shoppingcart)
            setloaded(true)
 ...
 const displayoutofstockerr =()=>{
  ...
  const updatequantity=(data)=>{
    ...
    if(mobiledevice){
      ...
      setquantityalertid("")
   axios.get(`https://new-frugetbackend-production.up.railway.app/client/updatequantity?shoppingcartId=${data.id}&productId=${data.productId}&action=${data.action}&tkt=${Cookies.get("tktplc")}`)
   .then( res =>{
 if(res.data.status === "success"){
  var totalregdispatch =0;
  var totalcurdispatch=0;
  ...
  const deletecart =(shoppingcartId)=>{
    ...
    setwarningdetails({})
    setloading(true)
   axios.get(`https://new-frugetbackend-production.up.railway.app/client/deletecart?shoppingcartId=${shoppingcartId}&tkt=${Cookies.get("tktplc")}`)
   .then( res =>{
 if(res.data.status === "success"){
  var totalcurdispatch=0;
  ...
  const saveItem=(data)=>{  
   ...
   if(Cookies.get("tktplc")){
    ...
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/save_item?tkt=${Cookies.get("tktplc")}&productId=${data.productId}`)
    .then(res =>{
        if(res.data.status === "success"){
...
const unsaveItem=(data)=>{  
   ...
   if(Cookies.get("tktplc")){
    setloading(true)
    const tkt = Cookies.get("tktplc")
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/unsave_item?tkt=${Cookies.get("tktplc")}&productId=${data.productId}`)
    .then(res =>{
        if(res.data.status === "success"){
            setsavedProducts(res.data.savedItems)
...
const submitcart =()=>{
   setloading(true)
    axios.get(`https://new-frugetbackend-production.up.railway.app/client/submit_cart?tkt=${Cookies.get("tktplc")}&sumtotalmain=${sumtotalmain}&paymenttype=${paymenttype}`)
    .then(res =>{
       if(res.data.status === "success"){
       setshoppingcart(res.data.shoppingcart)
